...
`BOT_MATCH_SERVER_URL` is intentionally empty: the assignment's `server_url` is used. The remote queue endpoint must be HTTPS and have the same host as `ORCHESTRATOR_URL`; localhost may use HTTP/WS for debug. Queue redirects are rejected.
